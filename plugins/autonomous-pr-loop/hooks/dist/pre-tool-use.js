...
function matchesGitLsRemoteAllowlist(args) {
  ...
  return args.slice(1).every(
    (arg) => arg === "main" || arg === "refs/heads/main" || /^refs\/tags\/v\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(arg) || isCodexBranch(arg)
  );
}
...
function matchesGhRepoViewAllowlist(args, repoId) {
...
function matchesGhReleaseReadAllowlist(args, repoId) {
